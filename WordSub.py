...
Use the sub() method to perform the substitution:
    > print_ subber.sub("before we begin")
    after we end
...
The following example illustrates this point:
    > subber["he"] = "she"
    > print_ subber.sub("he says he'd like to help her")
...
if __name__ == "__main__":
    ...
    if subber.sub(inStr) == outStr:
        print_("Test #1 PASSED")
    else:
        print_("Test #1 FAILED: '%s'" % subber.sub(inStr))

    inStr = "He said he'd like to go with me"
    outStr = "She said she'd like to go with me"
    if subber.sub(inStr) == outStr:
        print_("Test #2 PASSED")
    else:
        print_("Test #2 FAILED: '%s'" % subber.sub(inStr))
