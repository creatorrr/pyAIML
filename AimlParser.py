...
class AimlHandler(ContentHandler):
    ...
    def startElementNS(self, name, qname, attr):
        ...
        uri, elem = name
        if (elem == "bot"):
            print_("name:", attr.getValueByQName("name"), "a'ite?")
    ...
    def _startElement(self, name, attr):
        if name == "aiml":
            ...
            try:
                ...
                # print_ "WARNING: Missing 'version' attribute in <aiml> tag "+self._location()
                # print_ "         Defaulting to version 1.0"
                self._version = "1.0"
            ...
            try:
                ...
                raise AimlParserError(
                    "Required \"name\" attribute missing in <topic> element " + self._location())
    ...
    def _characters(self, ch):
        ...
        if self._state == self._STATE_InsidePattern:
            ...
            try:
                ...
                if not canBeParent:
                    raise AimlParserError(
                        ("Unexpected text inside <%s> element " % parent) + self._location())
    ...
    def _validateElemStart(self, name, attr, version):
        ...
        for a in attr:
            ...
            if a not in optional and not self._forwardCompatibleMode:
                raise AimlParserError(
                    ("Unexpected \"%s\" attribute in <%s> element " % (a, name)) + self._location())
            ...
            for k, v in list(attr.items()):
                if k == "index":
                    ...
                    try:
                        ...
                        raise AimlParserError(
                            ("Bad type for \"%s\" attribute (expected integer, found \"%s\") " % (k, v)) + self._location())
                    if temp < 1:
                        raise AimlParserError(
        ...
        try:
            ...
            # happen.
            raise AimlParserError(
                ("Element stack is empty while validating <%s> " % name) + self._location())
        required, optional, canBeParent = self._validInfo[parent]
        nonBlockStyleCondition = (parent == "condition" and not (
            "name" in parentAttr and "value" in parentAttr))
        if not canBeParent:
        ...
        # contain <li> subelements.
            ...
            raise AimlParserError(
                ("<%s> elements can only contain <li> subelements " % parent) + self._location())
